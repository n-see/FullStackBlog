...
const GetLoggedInUser = async (username) => {
    let result = await fetch(
        `http://localhost:5118/api/User/GetUserByUsername/${username}`
    );
...
const sendData = async (controller, endpoint, passedInData) => {
    const result = await fetch(
        `http://localhost:5118/api/${controller}/${endpoint}`,
        {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
...
const GetItemsByUserId = async (UserId) => {
    let result = await fetch(
        `http://localhost:5118/api/blog/GetItemsByUserId/${UserId}`
    );
